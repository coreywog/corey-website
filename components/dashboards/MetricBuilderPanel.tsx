...
import { CalculatedMetricForm, type DraftMetricFields } from "./CalculatedMetricForm";
...
type PreviewState =
  ...
  | { status: "ready"; value: number; sampleSize: number; label?: string; transactions?: { merchant: string; amount: number }[] };
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setPreview({ status: "ready", value: body.value, sampleSize: body.sampleSize, label: body.label, transactions: body.transactions });
        } else {
        ...
        {preview.status === "ready" && draft && (
          <>
            ...
            {preview.transactions && preview.transactions.length > 0 && (
              <ul className="mt-2 flex flex-col gap-0.5 text-xs text-zinc-600 dark:text-zinc-300">
